...
namespace Noble.Report.NobleDefaultServices
{
    public static class GetAllAccounts
    {
        ...
        public static List<AllAccountsLookupModel> GetAllAccountsDtl (string token,string serverName) {
            ...
            RestRequest request1 = new RestRequest("Report/GetAllAccounts"); 

            // add the token to the Authorization header of the request
            request1.AddHeader("Authorization", "Bearer " + token);
            var response1 = client1.Execute(request1);
            var content1 = response1.Content;
          var   AccountDtl = JsonConvert.DeserializeObject<List<AllAccountsLookupModel>>(content1);
