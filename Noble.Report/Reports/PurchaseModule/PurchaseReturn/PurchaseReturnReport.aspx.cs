...
using Noble.Report.Reports.Invoice.PurchaseReturn;
using System;
using System.IO;
using System.Linq;
using System.Web;

namespace Noble.Report.Reports.PurchaseModule.PurchaseReturn
{
    public partial class PurchaseReturnReport : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (printSetting.PrintTemplate == "Default")
                {
                    ...
                    if (Convert.ToBoolean(isDownload))
                    {
                        ...
                        XtraReport report1 = new PurchaseReturnDefaultTemplate(companyInfo, printSetting, purchaseDetails);
                ...
                else if (printSetting.PrintTemplate == "Template6")
                {
                    if (Convert.ToBoolean(isDownload))
                    {
                        ...
                        XtraReport report1 = new PurchaseReturnTemplate6(companyInfo, printSetting, purchaseDetails);
                        report1.CreateDocument();
                        using (var stream = new MemoryStream())
                        {
                            report1.ExportToPdf(stream);
                    ...
                    else
                    {
                        ASPxWebDocumentViewer1.Visible = true;
                        ASPxGridView1.Visible = false;
                        XtraReport temp6 = new PurchaseReturnTemplate6(companyInfo, printSetting, purchaseDetails);
                ...
                else
                {
                    if (Convert.ToBoolean(isDownload))
                    {
                        ...
                        XtraReport report1 = new Invoice.PurchaseReturn.PurchaseReturn(companyInfo, printSetting, purchaseDetails);
                        report1.CreateDocument();
                        using (var stream = new MemoryStream())
                        {
                            report1.ExportToPdf(stream);
