    ...
    <div class="row" v-if="isValid('CanAddSaleReturn')">
        <div class="col-lg-12">
                ...
                <div class="col-sm-12">
                    <div class="page-title-box">
                        <div class="row">
                            <div class="col">
                                <h4 class="page-title">
                                    {{ $t('AddSaleReturn.ServiceSaleReturn') }}
                                </h4>
            ...
            <div class="row" v-if="isDayAlreadyStart">
                <div class="col-lg-6">
                    <div class="row form-group" v-if="!sale.isCredit">
                        ...
                        <div class="inline-fields col-lg-8">
                            <customerdropdown v-model="v$.sale.customerId.$model" @update:modelValue="GetInvoiceList" :modelValue="sale.customerId" v-bind:key="rendered" />
                        </div>
                    </div>
                    ...
                    <div class="row form-group">
                        ...
                        <div class="inline-fields col-lg-8">
                            <sale-invoice-dropdown v-model="sale.saleInvoiceId" @update:modelValue="getSaleDetail"
                                :isService="true" :isReturn="true" :customerId="sale.customerId" :key="saleListRender" />
                        </div>
                    </div>
                    ...
                    <div class="row form-group">
                        ...
                        <div class="inline-fields col-lg-8">
                            <input type="text" class="form-control" disabled @focus="$event.target.select()"
                ...
                <div class="col-lg-6">
                    ...
                    <div class="row form-group">
                        <label class="col-form-label col-lg-4">
                            <span class="tooltip-container text-dashed-underline"
                                :class="{ '': sale.isCredit, '': !sale.isCredit }">
                    ...
                    <div class="row form-group"
                        ...
                        <div class="inline-fields col-lg-8">
                            <multiselect v-if="this.$i18n.locale == 'en'" :options="['Inclusive', 'Exclusive']"
                                v-bind:disabled="sale.saleItems.length > 0" v-model="sale.taxMethod" :show-labels="false"
                                v-bind:placeholder="$t('AddStockValue.SelectMethod')">
                            </multiselect>
                            <multiselect v-else :options="['شامل', 'غير شامل']" v-bind:disabled="sale.saleItems.length > 0"
                                v-model="sale.taxMethod" :show-labels="false"
                                v-bind:placeholder="$t('AddStockValue.SelectMethod')">
                            </multiselect>
                        </div>
                    </div>
                    ...
                    <div class="row form-group"
                        ...
                        <div class="inline-fields col-lg-8">
                            <taxratedropdown v-model="sale.taxRateId" v-bind:modelValue="sale.taxRateId"
                                :isDisable="sale.saleItems.length > 0 ? true : false" v-bind:key="rendered" />
                        </div>
                    </div>

                </div>

                <!-- <sale-service-item :saleItems="sale.saleItems" @update:modelValue="SaveSaleItems" @summary="updateSummary" :key="rendered" :formName="'saleReturn'" /> -->

                <sale-service-item :saleItems="sale.saleItems" @update:modelValue="SaveSaleItems" @summary="updateSummary"
                    :key="rendered" :formName="'saleReturn'" @discountChanging="updateDiscountChanging"
                    :newtaxMethod="sale.taxMethod" :taxRateId="sale.taxRateId" :adjustmentProp="sale.discount"
                    :adjustmentSignProp="adjustmentSignProp" :isDiscountOnTransaction="sale.isDiscountOnTransaction"
                    :transactionLevelDiscountProp="sale.transactionLevelDiscount" :isFixed="sale.isFixed"
                    :isBeforeTax="sale.isBeforeTax" :wareHouseId="sale.wareHouseId" />
                ...
                <div class="col-lg-12 invoice-btn-fixed-bottom">
                    <div v-if="sale.id === '00000000-0000-0000-0000-000000000000'">
                        <button class="btn btn-outline-primary  me-2" v-on:click="saveSaleReturn('SaleReturn', true, false)"
                            v-if="isValid('CanAddSaleReturn')"
                            v-bind:disabled="v$.sale.$invalid || sale.saleItems.filter(x => x.outOfStock).length > 0 || sale.saleItems.filter(x => x.totalPiece == '').length > 0 || sale.saleItems.filter(x => x.unitPrice == '').length > 0">
                            <i class="far fa-save"></i> {{ $t('AddSaleReturn.SaveAsPost') }}
                        </button>
                        <button class="btn btn-outline-primary  me-2" v-on:click="saveSaleReturn('SaleReturn', true, true)"
                            v-if="isValid('CanAddSaleReturn') && isValid('CanPrintInvoice')"
                            v-bind:disabled="v$.sale.$invalid || sale.saleItems.filter(x => x.outOfStock).length > 0 || sale.saleItems.filter(x => x.totalPiece == '').length > 0 || sale.saleItems.filter(x => x.unitPrice == '').length > 0">
                            <i class="far fa-save"></i> {{ $t('AddSaleReturn.SaveasPostandPrint') }}
                        </button>
                        <button class="btn btn-danger  me-2" v-on:click="goToSale">
                ...
                <div class="card p-3 text-center " v-if="bankDetail">
                    ...
                    <router-link
                        :to="{ path: '/WholeSaleDay', query: { token_name: 'DayStart_token', fromDashboard: 'true' } }"><a
                            href="javascript:void(0)" class="btn btn-outline-danger ">{{ $t('AddSaleReturn.DayStart')
                            }}</a></router-link>
                </div>
            ...
            <saleReturnReport :printDetails="printDetails" :headerFooter="headerFooter"
                v-if="printDetails.length != 0 && printTemplate != 'Template6'" :isTouchScreen="saleReturn"
                v-bind:key="printRender"></saleReturnReport>
...
<script>
    ...
    export default {
            ...
            return {
                sale: {
                    ...
                    cashCustomer: {
                        ...
                        required: requiredIf(function () {
                            return !this.isCredit && (this.sale.customerId === '00000000-0000-0000-0000-000000000000' || this.sale.customerId === null);
                        }),
        ...
        methods: {
            GetInvoiceList: function (id, accountId, customer) {
                if (customer == null) {
                    return
                }
                this.saleListRender++;
                this.sale.mobile = customer.contactNo1;
                this.sale.cashCustomer = customer.customerDisplayName;
                ...
                root.$https.get('/Sale/SaleDetail?Id=' + id + '&isFifo=' + this.isFifo + '&openBatch=' + this.openBatch, { headers: { "Authorization": `Bearer ${token}` } })
                    .then(function (response) {
                        if (response.data != null) {
                            ...
                            root.sale.cashCustomer = response.data.customerDisplayName;
                            ...
                            root.sale.isService = response.data.isService;
                            root.sale.termAndCondition = response.data.termAndCondition;
                            root.sale.termAndConditionAr = response.data.termAndConditionAr;
                            root.sale.warrantyLogoPath = response.data.warrantyLogoPath;
            ...
            AutoIncrementCode: function () {
                ...
                root.$https
                    .get("/Sale/SaleAutoGenerateNo?invoiceType=" + "SaleReturn" + '&branchId=' + localStorage.getItem('BranchId') + '&terminalId=' + terminalId, {
                        headers: { Authorization: `Bearer ${token}` },
                    })
                    .then(function (response) {
                        if (response.data != null) {
            ...
            saveSaleReturn: function (invoiceType, status, print) {
                ...
                this.$https.post('/Sale/SaveSaleReturn', root.sale, { headers: { "Authorization": `Bearer ${token}` } })
                    .then(response => {
        ...
        created: function () {
            ...
            if (this.$route.query.data != undefined) {
                ...
                this.sale.cashCustomer = data.customerDisplayName;
