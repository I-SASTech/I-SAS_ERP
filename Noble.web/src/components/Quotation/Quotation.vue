<template>
    <div class="row"
         v-if="(isValid('CanDraftQuotation') || isValid('CanViewQuotation'))">
        <div class="col-lg-12">
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-title-box">
                        <div class="row">
                            ...
                            <div class="col-auto align-self-center">
                                <a v-if="isValid('CanAddQuotation') || isValid('CanDraftQuotation')"
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs" role="tablist">
                        ...
                        <li class="nav-item"
                            v-if="isValid('CanViewQuotationTemplate')"
                            v-on:click="makeActive('QuotationTemplate')">
                            <a class="nav-link"
                               v-bind:class="{active:active == 'QuotationTemplate'}" data-bs-toggle="tab" href="#profile" role="tab" aria-selected="false">
                                ...
                                <table class="table mb-0">
                                    ...
                                    <tbody>
                                        <tr v-for="(purchaseOrder,index) in saleOrderList" v-bind:key="purchaseOrder.id">
                                            ...
                                            <td v-if="isValid('CanEditQuotation')">
                                                <strong>
                                            ...
                                            <td class="text-end">
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item" href="javascript:void(0)" v-on:click="EditPurchaseOrder(purchaseOrder.id)" v-if="isValid('CanEditQuotation')">{{ $t('Quotation.EditInvoice') }}</a>
                                                    <a class="dropdown-item" href="javascript:void(0)" v-on:click="ViewInvoice(purchaseOrder.id)" v-if="isValid('CanViewQuotationDetail')">{{ $t('Quotation.ViewQuotation') }}</a>
                                                    <a class="dropdown-item" href="javascript:void(0)" v-on:click="PrintInvoice(purchaseOrder.id)" v-if="isValid('CanPrintQuotation')">{{ $t('Quotation.A4Print') }}</a>
                                                    <a class="dropdown-item" href="javascript:void(0)" v-on:click="PrintInvoicePdf(purchaseOrder.id)" v-if="isValid('CanPrintQuotation')">{{ $t('Quotation.PdfDownload') }}</a>
                                ...
                                <table class="table mb-0">
                                    ...
                                    <tbody>
                                        <tr v-for="(purchaseOrder,index) in saleOrderList" v-bind:key="purchaseOrder.id">
                                            ...
                                            <td>
                                                <a href="javascript:void(0)" v-on:click="ViewCustomerInfo(purchaseOrder.customerId)" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight">{{purchaseOrder.customerName == null? purchaseOrder.customerArabicName : purchaseOrder.customerName}}</a>
                                            </td>
                                            <td>
                                                {{currency}} {{parseFloat(purchaseOrder.netAmount).toFixed(3).slice(0,-1).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,")}}
                                            </td>
                                            <td>
                                                ...
                                                <span class="badge badge-boxed  badge-outline-success" v-if="purchaseOrder.isClose">{{ $t('Quotation.Close') }}</span>
                                            </td>
                                            <td class="text-end">
                                                ...
                                                <div class="dropdown-menu">
                                                    ...
                                                    <a class="dropdown-item" href="javascript:void(0)" v-on:click="PrintInvoice(purchaseOrder.id)" v-if="isValid('CanPrintQuotation')">{{ $t('Quotation.A4Print') }}</a>
                                                    <a class="dropdown-item" href="javascript:void(0)" v-on:click="PrintInvoicePdf(purchaseOrder.id)" v-if="isValid('CanPrintQuotation')">{{ $t('Quotation.PdfDownload') }}</a>
                                                    <a class="dropdown-item" href="javascript:void(0)" v-on:click="TemplateofQuotation(purchaseOrder.id)" v-if="isValid('CanAddQuotationTemplate')">{{ $t('Quotation.Template') }}</a>
 ...
 $t('Pagination.ShowingEntries')
        ...
        $t('Pagination.entries')
                        ...
                        <div class="tab-pane pt-3" id="profile" role="tabpanel" v-bind:class="{ active: active == 'QuotationTemplate' }">
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    <thead class="thead-light table-hover">
                                        <tr>
                                    ...
                                    <tbody>
                                        <tr v-for="(purchaseOrder,index) in saleOrderList" v-bind:key="purchaseOrder.id">
                                            ...
                                            <td>
                                                <strong>
                                                    <a href="javascript:void(0)" v-on:click="EditQuotationTemplate(purchaseOrder.id)">{{purchaseOrder.registrationNumber}}</a>
                                                </strong>
                                            </td>
                                            <td>
                                                {{purchaseOrder.date}}
    ...
    export default {
        ...
        methods: {
            ViewCustomerInfo: function (id) {
                ...
                root.$https.get('/Contact/ContactLedgerDetail?id=' + id + '&documentType=' + "Quotation" + '&isService=false', { headers: { "Authorization": `Bearer ${token}` } })
            ...
            makeActive: function (item) {
                ...
                this.selected = [];
                if (item == 'QuotationTemplate') {
                    this.getData1(this.search, 1, item);
                }
                else {
            ...
            getData1: function (search, currentPage, status) {
                ...
                this.$https.get('/Purchase/QuotationTemplateList?status=' + status + '&searchTerm=' + search + '&pageNumber=' + currentPage + '&isQuotation=true', { headers: { "Authorization": `Bearer ${token}` } })
                    .then(function (response) {
            ...
            getData: function (search, currentPage, status) {
                ...
                this.$https.get('/Purchase/SaleOrderList?status=' + status + '&searchTerm=' + search + '&pageNumber=' + currentPage + '&isQuotation=true', { headers: { "Authorization": `Bearer ${token}` } })
                    .then(function (response) {
