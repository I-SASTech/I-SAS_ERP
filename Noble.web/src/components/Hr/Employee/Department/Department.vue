<template>
                            ...
                            <thead class="thead-light table-hover">
                                <tr>
                                    ...
                                    <th v-if="english=='true'" width="20%">
                                        {{$englishLanguage($t('brand.BrandName'))  }}
                                    </th>
                                    <th width="20%">
                                        {{$arabicLanguage($t('brand.BrandNameArabic'))  }}
                                    </th>
                                    <th width="40%">
                                        {{ $t('brand.Description') }}
                                    </th>
 ...
 $t('Pagination.of')
            ...
            <departmentmodel :newDepartment="newBrand"
                        :show="show"
                        v-if="show"
                        @close="IsSave"
                        :type="type" />
    ...
    export default {
        ...
        clearData: function () {
            ...
            GetBrandData: function () {
                ...
                root.$https.get('/EmployeeRegistration/DepartmentList?isActive=false' + '&pageNumber=' + this.currentPage + '&searchTerm=' + this.search, { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
                    if (response.data != null) {
                        root.brandlist = response.data.results.departments;
            ...
            EditBrand: function (Id) {
                ...
                root.$https.get('/EmployeeRegistration/DepartmentDetail?Id=' + Id, { headers: { "Authorization": `Bearer ${token}` } })
                    .then(function (response) {
                        if (response.data) {
                            root.newBrand.id = response.data.id;
                            root.newBrand.name = response.data.name;
                            root.newBrand.bankId = response.data.bankId;
