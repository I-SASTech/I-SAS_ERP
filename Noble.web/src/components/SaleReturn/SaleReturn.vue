<template>
                                            ...
                                            <td class="text-end">
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item" href="javascript:void(0)"
                                                        ...
                                                        v-if="isValid('CanViewDetailSaleReturn')">{{
                                                            $t('SaleReturn.ViewInvoice') }}</a>
                                                    <a class="dropdown-item" href="javascript:void(0)"
                                                        v-on:click="PrintRdlc(sale.id, false)"
                                                        v-if="isValid('CanPrintInvoice')">{{ $t('SaleReturn.A4-Print')
                                                        }}</a>
                                                    <a class="dropdown-item" href="javascript:void(0)"
                                                        v-on:click="PrintRdlc(sale.id, false)"
                                                        v-if="isValid('CanPrintInvoice')">{{ $t('SaleReturn.POS-Print')
                                                        }}</a>
                                                    <a class="dropdown-item" href="javascript:void(0)"
                                                        v-on:click="PrintRdlc(sale.id, true)"
                                                        v-if="isValid('CanPrintInvoice')">{{ $t('SaleReturn.PdfDownload')
                                                        }}</a>
    ...
    $t('Pagination.entries')
            ...
            <!-- <saleReturnReport :printDetails="printDetails" :headerFooter="headerFooter" v-if="printDetails.length != 0  && show" v-bind:key="printRender"></saleReturnReport>
        <SaleReturnPdf :printDetails="printDetails" :headerFooter="headerFooter" v-if="printDetails.length != 0 && pdfShow" v-bind:key="printpdfRender" /> -->
        
        <loading v-model:active="loading" :can-cancel="true" :is-full-page="false"></loading>   
        
            <saleInvoice :printDetails="printDetails" :headerFooter="headerFooter" v-if="printDetails.length != 0 && show"
                v-bind:key="printRender" :formName="'SaleReturn'" />
        ...
        <div class="col-lg-6 col-sm-6 ml-auto mr-auto" v-else>
            <div class="card p-3 text-center " v-if="bankDetail">
                <h4 class="">{{ $t('SaleReturn.FirstStartInvoice') }}</h4>
                <router-link :to="{ path: '/WholeSaleDay', query: { token_name: 'DayStart_token', fromDashboard: 'true' } }"><a
                        href="javascript:void(0)" class="btn btn-outline-danger ">{{ $t('SaleReturn.DayStart')
                        }}</a></router-link>
            </div>
...
export default {
    ...
    data: function () {
        ...
        methods: {
           ...
           PrintRdlc: function (id, isDownload) {
            ...
            {
                this.loading=true;
                this.$https.get(this.$ReportServer + '/SalesModuleReports/SaleReturnReport/SaleReturnReport.aspx?Id=' + id + '&isFifo=' + isFifo + '&openBatch=' + openBatch + '&isReturn=' + true + '&deliveryChallan=' + false + '&simpleQuery=' + false + '&colorVariants=' + this.colorVariants + '&CompanyId=' + companyId + '&formName=ReturnInvoice' + '&isDownload=' + isDownload
                , {  responseType: 'blob' } ) .then(function (response) {
                    root.loading=false;
                        ...
                        link.setAttribute('download', 'ReturnInvoice  ' + date + '.pdf');
                ...
                this.reportsrc = this.$ReportServer + '/SalesModuleReports/SaleReturnReport/SaleReturnReport.aspx?Id=' + id + '&isFifo=' + isFifo + '&openBatch=' + openBatch + '&isReturn=' + true + '&deliveryChallan=' + false + '&simpleQuery=' + false + '&colorVariants=' + this.colorVariants + '&CompanyId=' + companyId + '&formName=ReturnInvoice' + '&isDownload=' + isDownload
                this.show = !this.show;
                this.changereport++;
            }
    ...
    var branchId = localStorage.getItem('BranchId');
        ...
        getData: function (search, currentPage, status, fromDate, toDate) {
            ...
            localStorage.setItem('currentPage', this.currentPage);
                ...
                this.$https.get('/Sale/SaleList?status=' + 'SaleReturn' + '&searchTerm=' + search + '&pageNumber=' + currentPage + '&isSaleReturnPost=' + isSaleReturnPost + '&fromDate=' + fromDate + '&toDate=' + toDate + '&branchId=' + branchId, { headers: { "Authorization": `Bearer ${token}` } })
                    .then(function (response) {
                        if (response.data != null) {
    ...
    mounted: function () {
        ...
        if (IsDayStart != 'true') {
            ...
            this.search = '';
            //this.active = this.$route.query.active == undefined ? 'Draft' : this.$route.query.active;
