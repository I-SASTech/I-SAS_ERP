        ...
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title m-0" id="exampleModalDefaultLabel" v-if="type == 'Edit'">
                    {{
                    $t('AddProductGroup.UpdateCustomerGroup')
                    }}
                </h6>
                <h6 class="modal-title m-0" id="exampleModalDefaultLabel" v-else>
                    {{
 $t('AddProductGroup.AddCustomerGroup')
                    }}
                </h6>
                ...
                <div class="row">
                    ...
                    <div v-if="english == 'true'" class="form-group has-label col-sm-4 ">
                        <label class="text  font-weight-bolder">
                            {{$englishLanguage($t('AddProductGroup.Name'))   }}:<span class="text-danger"> *</span>
                        </label>
                        <input class="form-control" v-model="v$.color.name.$model" />
                    </div>

                    <div v-if="isOtherLang()" class="form-group has-label col-sm-4 "
                         v-bind:class="{ 'has-danger': v$.color.nameArabic.$error }">
                        <label class="text  font-weight-bolder">
                            {{$arabicLanguage($t('AddProductGroup.NameArabic'))   }}:<span class="text-danger"> *</span>
                        </label>
                        <input class="form-control " v-model="v$.color.nameArabic.$model" type="text" />
                    </div>

                    <div class="form-group has-label col-sm-12 "
                         v-bind:class="{ 'has-danger': v$.color.description.$error }">
                        ...
                        <span v-if="v$.color.description.$error" class="error">
                            {{
 $t('AddColors.descriptionLength')
                            }}
                        </span>
                    </div>
                    ...
                    <div class="form-group has-label col-sm-12">
                        <customerdropdown @update:modelValue="addProduct" width="100%" />
                        ...
                        <table class="table mt-3">
                            ...
                            <tbody>
                                <tr v-for="(color, index) in color.contactsList" v-bind:key="index">
                                    <td>
                                        {{ index + 1 }}
                                    </td>
                                    <td>{{ color.code }}</td>
                                    ...
                                    <td class="text-end">
                                        <a href="javascript:void(0);" @click="removeProduct(index)">
    ...
    export default {
        ...
        methods: {
            addProduct: function (productId, newProduct) {
                ...
                var prod = this.color.contactsList.find((x) => x.id == productId);

                if (prod == undefined) {
                    if (newProduct.code != undefined) {
                        this.color.contactsList.push({
                            contactId: productId,
                            name: newProduct.customerDisplayName,
            ...
            removeProduct: function (index) {
                ...
                this.color.contactsList.splice(index, 1);
            },
            // removeProduct: function (id) {
            //     this.color.contactsList = this.color.contactsList.filter((prod) => {
            ...
            GetAutoCodeGenerator: function () {
                ...
                this.$https.get('/Contact/CustomerGroupAutoCode', { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
            ...
            SaveColor: function () {
                ...
                this.$https.post('/Contact/SaveCustomerGroup', this.color, { headers: { "Authorization": `Bearer ${token}` } })
