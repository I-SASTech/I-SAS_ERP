    ...
    <div id="matchingTransactionTable">
        <modal :show="show" :modalLarge="true">
            <div class="modal-header">
                <h5 class="modal-title DayHeading" id="myModalLabel">{{ $t('AssignPermissionsToRole.Permissions') }}</h5>
                <small>{{ $t('AssignPermissionsToRole.For') }} {{roleDetails.name}}</small>
            </div>
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-md-7 mt-2">
                        <permissionCategoryDropdown v-bind:isTrue=true  v-model="selectedCategory"  v-bind:roleDetails="roleDetails" @update:modelValue="GetCategoryPermissionQuery"></permissionCategoryDropdown>
                        <!--<moduleNamesDropdown  @update:modelValue="GetRights(roleDetails.moduleId)"></moduleNamesDropdown>-->
                    </div>
                    ...
                    <table class="table table-striped table-hover">
                        <thead>
                            <tr>
                                ...
                                <th>{{ $t('AssignPermissionsToRole.Description') }}</th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
            ...
            <div class="modal-footer justify-content-center">
                <button type="button" class="btn btn-danger " v-on:click="close()">{{ $t('AssignPermissionsToRole.Close') }}</button>
                <button title="Add New Item" class="btn btn-outline-primary "
                        v-on:click="SavePermissionData()">
                    <i class="fa fa-plus"> </i> {{ $t('AssignPermissionsToRole.UpdateNew') }}
                </button>
            </div>
        </modal>
    </div>
       ...
       /* components: { VueRadioButton },*/
        ...
        methods: {
            GetCategoryPermissionQuery: function () {
                ...
                root.$https.get('/Company/GetByCategoryPermissionQuery?categoryName=' + this.selectedCategory + '&isRights=true' + '&roleId=' + this.roleDetails.roleId + '&companyId=' + this.roleDetails.companyId, { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
                    if (response.data.length > 0) {
                        ...
                        root.userRolesList.forEach(function (item) {
                            if (item.isActive) {
                                root.counter += 1;
                            }
                        })
                        if (root.counter == root.userRolesList.length) {
                            root.selectAll = true;
                        }
                        else {
            ...
            GetRights: function (id){
                ...
                //eslint-disable-line
                root.$https.get('/Company/GetRightsByModuleName?moduleName=' + id + '&isActive=true' + '&roleId=' + this.roleDetails.roleId, { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
                    if (response.data.length > 0) {
                        root.userRolesList = response.data;
                        root.isShown = true;
                    }
            ...
            SavePermissionData: function () {
                ...
                if(this.allowAll == ''){
                    for (let i in this.userRolesList) {
                        nobleRolePermission.push({
                            ...
                            permissionId: this.userRolesList[i].permissionId,
                            isActive: this.userRolesList[i].isActive,
               ...
               //eslint-disable-line
                ...
                root.$https.post('/Company/SaveRolePermissions', nobleRolePermission, { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
                   //eslint-disable-line
                    root.allowAll = '';
