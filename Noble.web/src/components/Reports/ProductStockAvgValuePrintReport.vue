                ...
                <div class="row">
                    ...
                    <div class="col-md-4 text-center my-5" style="padding:0px !important; margin:0 !important">
                        ...
                        <p style="text-align: center; margin: 0px; padding: 0px; font-size: 10px; line-height: 1; font-family: 'Times New Roman', Times;">
                            <span style="font-size:14px;">{{ $t('ProductStockAvgValuePrintReport.StockReport') }}</span>
                        </p>
                    </div>
            ...
            <div class="border border-dark col-md-12 my-1 " style="height:200mm;">
                ...
                <div class="row">
                    ...
                    <h6 class="m-auto" v-if="list.inventoryList.length > 0 && isShown">
                        {{ $t('ProductStockAvgValuePrintReport.WareHouseName') }} : {{list.inventoryList[0].wareHouseName}} || {{ $t('ProductStockAvgValuePrintReport.ProductName') }} : {{list.inventoryList[0].productName}}
                    </h6>
                    <table class="table col-md-11 m-auto">
                        <tr class="heading" style="font-size:14px;">
                            <th class="text-left">#</th>
                            <th class="text-left">{{ $t('ProductStockAvgValuePrintReport.Date') }}</th>
                            <th class="text-left">{{ $t('ProductStockAvgValuePrintReport.TransactionType') }}</th>
                            <th v-if="!isShown" class="text-left">{{ $t('ProductStockAvgValuePrintReport.ProductName') }}</th>
                            <th v-if="!isShown" class="text-left">{{ $t('ProductStockAvgValuePrintReport.WareHouseName') }}</th>
                            <th class="text-right">{{ $t('ProductStockAvgValuePrintReport.QuantityIn') }}</th>
                            <th class="text-right">{{ $t('ProductStockAvgValuePrintReport.QuantityOut') }}</th>
                            <th class="text-right">{{ $t('ProductStockAvgValuePrintReport.AvgPurchasePrice') }}</th>
                            <th class="text-right">{{ $t('ProductStockAvgValuePrintReport.AvgSalePrice') }}</th>
                            <th class="text-right">{{ $t('ProductStockAvgValuePrintReport.LineTotal') }}</th>
                        </tr>
                        <tr style="font-size:13px;" v-for="(item, index) in list.inventoryList" v-bind:key="item.id">
                            ...
                            <td v-if="isMultiUnit == 'true'" class="text-right">
                            ...
                            <td class="text-right">
                                {{item.transactionType=='StockOut'?0:item.transactionType=='StockIn'?item.avgPurchasePrice:item.transactionType=='SaleInvoice'?0:item.transactionType=='PurchaseReturn'?item.avgPurchasePrice:item.transactionType=='PurchasePost'?item.avgPurchasePrice:item.transactionType=='CashReceipt'?0:item.transactionType=='CashReceipt'?0:''}}
                            </td>
                            <td class="text-right">
                                {{item.transactionType=='StockOut'?item.avgSalePrice:item.transactionType=='StockIn'?0:item.transactionType=='SaleInvoice'?item.avgSalePrice:item.transactionType=='PurchaseReturn'?0:item.transactionType=='PurchasePost'?0:item.transactionType=='CashReceipt'?0:item.transactionType=='CashReceipt'?0:''}}
                            </td>
                            <td class="text-right">
                                {{parseFloat(item.quantityOut * item.avgSalePrice).toFixed(3).slice(0,-1)}}
                            </td>
                        </tr>
                    </table>
                </div>
            ...
            <div class="col-md-12  " style="height:30mm;">
                <div class="row">
                    <table class="table text-center">
                        <tr>
                            <td>
                                {{ $t('ProductStockAvgValuePrintReport.ReceivedBy') }}:
                            </td>
                            <td>
                                {{ $t('ProductStockAvgValuePrintReport.ApprovedBy') }}:
                            </td>
                            <td>
                                {{ $t('ProductStockAvgValuePrintReport.PreparedBy') }}:
                            </td>
                        </tr>
    ...
    export default {
        ...
        computed: {
            calulateTotalPrice: function() {
                return this.list.inventoryList.reduce(function(a, c){return a + Number((c.quantityOut * c.avgSalePrice))}, 0)
            }
        },
        mounted: function () {
            var root = this;
