<template>
    <div  v-if="isValid('CanViewSuppliersWiseProductPurchase')" v-bind:style="$i18n.locale == 'ar' ? languageChange('en') : languageChange('ar')">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group">
                                    <h5 class="card-title DayHeading">{{ $t('SuppliersWiseProductPurchaseReport.SuppliersWiseProductPurchaseReport') }}</h5>
                                </div>
                            </div>
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group " v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? 'text-right' : 'text-left'">
                                    ...
                                    <a href="javascript:void(0)" v-if="isValid('CanPrintSuppliersWiseProductPurchase')" class="btn btn-outline-primary " v-on:click="PrintDetails">{{ $t('SuppliersWiseProductPurchaseReport.Print') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row" v-bind:key="render" v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? '' : 'pr-3'">
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ml-3">
                                <label>{{ $t('SuppliersWiseProductPurchaseReport.FromDate') }}</label>
                                <datepicker v-model="fromDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group">
                                <label>{{ $t('SuppliersWiseProductPurchaseReport.ToDate') }}</label>
                                <datepicker v-model="toDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group">
                                <label>{{ $t('SuppliersWiseProductPurchaseReport.ProductName') }}</label>
                                ...
                                <table class="table table-striped table-hover table_list_bg">
                                    <thead class="">
                                        <tr>
                                            <th>#</th>
                                            <th>
                                                {{ $t('SuppliersWiseProductPurchaseReport.TransactionType') }}
                                            </th>
                                            <th>
                                                {{ $t('SuppliersWiseProductPurchaseReport.Customer') }}
                                            </th>
                                            <th>
                                                {{ $t('SuppliersWiseProductPurchaseReport.Quantity') }}
                                            </th>
                                            <th>
                                                {{ $t('SuppliersWiseProductPurchaseReport.PurchasePrice') }}
                                            </th>
                                            <th>
                                                {{ $t('SuppliersWiseProductPurchaseReport.LineTotal') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(product,index) in productList" v-bind:key="product.id">
                                            ...
                                            <td>
                                                {{parseFloat(product.quantityIn * product.purchasePrice).toFixed(3).slice(0,-1)}}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                productList: [],
                isShown: false,
                formName: 'Suppliers Wise Product Purchase Report',
                printRender: 0,
                combineDate: '',
                language: 'Nothing',
                isMultiUnit: '',
        ...
        methods: {
            languageChange: function (lan) {
                if (this.language == lan) {
                    ...
                    this.$router.go('/SuppliersWiseProductPurchaseReport');
            ...
            productValue: function (x) {
                var productid = x;
                this.GetInventoryList(this.fromDate, this.toDate, this.currentPage, productid);
            },
            getDate: function (date) {
                return moment(date).format('l');
            },
            getPage: function () {
                this.GetInventoryList(this.fromDate, this.toDate, this.currentPage, this.productId);
            },
            GetInventoryList: function (fromdate, todate, currentPage, productId) {  
                var root = this;
                var token = '';
                if (token == '') {
                    token = localStorage.getItem('token');
                }
                this.$https.get('/Company/GetInventoryList?fromDate=' + fromdate + '&toDate='+ todate + '&pageNumber=' + currentPage + '&isSuppliersWP=true' + '&productId=' + productId, { headers: { "Authorization": `Bearer ${token}` } })
            ...
            getTransactionType(transactionType){
                if (transactionType == 'StockOut') return this.$t('SuppliersWiseProductPurchaseReport.StockOut')
                if (transactionType == 'StockIn') return this.$t('SuppliersWiseProductPurchaseReport.StockIn')
                if (transactionType == 'SaleInvoice') return this.$t('SuppliersWiseProductPurchaseReport.SaleInvoice')
                if (transactionType == 'PurchaseReturn') return this.$t('SuppliersWiseProductPurchaseReport.PurchaseReturn')
                if (transactionType == 'PurchasePost') return this.$t('SuppliersWiseProductPurchaseReport.Purchase')
                if (transactionType == 'CashReceipt') return this.$t('SuppliersWiseProductPurchaseReport.Cash')
            },
