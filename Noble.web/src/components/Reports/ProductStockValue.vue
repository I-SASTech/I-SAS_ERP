<template>
    <div v-if="isValid('CanViewStockValueReport')" v-bind:style="$i18n.locale == 'ar' ? languageChange('en') : languageChange('ar')">
        <div >
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group ">
                                    <h5 class="card-title DayHeading">{{ $t('ProductStockValue.ProductStockValue') }}</h5>
                                </div>
                            </div>
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group " v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? 'text-right' : 'text-left'">
                                    ...
                                    <a href="javascript:void(0)" class="btn btn-outline-primary " v-if="isValid('CanPrintStockValueReport')" v-on:click="PrintDetails">{{ $t('ProductStockValue.Print') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row" v-bind:key="render" v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? '' : 'pr-3' ">
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ml-3">
                                <label>{{ $t('ProductStockValue.FromDate') }}</label>
                                <datepicker v-model="fromDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group">
                                <label>{{ $t('ProductStockValue.ToDate') }}</label>
                                    ...
                                    <thead class="">
                                        <tr>
                                            ...
                                            <th v-if="!advanceFilters">
                                                {{ $t('ProductStockValue.Product') }}
                                            </th>
                                            <th v-if="!advanceFilters">
                                                {{ $t('ProductStockValue.WareHouse') }}
                                            </th>
                                            <th>
                                                {{ $t('ProductStockValue.QuantityIn') }}
                                            </th>
                                            <th>
                                                {{ $t('ProductStockValue.QuantityOut') }}
                                            </th>
                                            <th>
                                                {{ $t('ProductStockValue.PurchasePrice') }}
                                            </th>
                                            <th>
                                                {{ $t('ProductStockValue.SalePrice') }}
                                            </th>
                                            <th>
                                                {{ $t('ProductStockValue.LineTotal') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(product,index) in productList" v-bind:key="product.id">
                                            ...
                                            <td v-if="!advanceFilters">
                                                ...
                                                <!-- <span v-else>{{product.wareHouseNameArabic}}</span> -->
                                                <br />
        ...
        methods: {
            ...
            languageChange: function (lan) {
                if (this.language == lan) {
                    ...
                    this.$router.go('/ProductStockValue');
            ...
            GetInventoryList: function (fromdate, todate, currentPage, productId, warehouseId) {  
                ...
                this.$https.get('/Company/GetInventoryList?fromDate=' + fromdate + '&toDate='+ todate + '&pageNumber=' + currentPage + '&isPSValue=true' + '&productId=' + productId + '&warehouseId=' + warehouseId, { headers: { "Authorization": `Bearer ${token}` } })
                .then(function (response) {
                    if (response.data != null) {
            ...
            getTransactionType(transactionType){
                if (transactionType == 'StockOut') return this.$t('ProductStockValue.StockOut')
                if (transactionType == 'StockIn') return this.$t('ProductStockValue.StockIn')
                if (transactionType == 'SaleInvoice') return this.$t('ProductStockValue.SaleInvoice')
                if (transactionType == 'PurchaseReturn') return this.$t('ProductStockValue.PurchaseReturn')
                if (transactionType == 'PurchasePost') return this.$t('ProductStockValue.Purchase')
                if (transactionType == 'CashReceipt') return this.$t('ProductStockValue.Cash')
            }
        },
        mounted: function () {
