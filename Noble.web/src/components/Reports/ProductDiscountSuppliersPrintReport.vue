        ...
        <div hidden id='inventoryDetailReport' class="col-md-7">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 text-center my-5" style="height:40mm;">
                        ...
                        <p style="text-align: center; margin: 0px; padding: 0px; font-size: 10px; line-height: 1; font-family: 'Times New Roman', Times;">
                            <span style="font-size:14px;">{{formName}}</span>
                        </p>
                    </div>
            ...
            <div class="border border-dark col-md-12 my-1 " style="height:200mm;">
                ...
                <div class="row">
                    ...
                    <h6 class="m-auto" v-if="list.inventoryList.length > 0 && isPrint">
                       Supplier Name : {{list.inventoryList[0].supplierName}}
                    </h6>
                    <table class="table col-md-11 m-auto">
                        <tr class="heading" style="font-size:14px;">
                            <th class="text-left">#</th>
                            <th class="text-left">{{ $t('ProductDiscountSuppliersPrintReport.Date') }}</th>
                            <th class="text-left">{{ $t('ProductDiscountSuppliersPrintReport.ProductName') }}</th>
                            <th class="text-right">{{ $t('ProductDiscountSuppliersPrintReport.QuantityIn') }}</th>
                            <th class="text-right">{{ $t('ProductDiscountSuppliersPrintReport.PurchasePrice') }}</th>
                            <th v-if="isShown" class="text-right">{{ $t('ProductDiscountSuppliersPrintReport.FixedDiscount') }}</th>
                            <th v-if="!isShown" class="text-right">{{ $t('ProductDiscountSuppliersPrintReport.PercentageDiscount') }}</th>
                        </tr>
                        <tr style="font-size:13px;" v-for="(item, index) in list.inventoryList" v-bind:key="item.id">
                            ...
                            <td v-if="isShown" class="text-right">
                                {{item.discountFixed}}
                            </td>
                            <td v-if="!isShown" class="text-right">
            ...
            <div class="col-md-12  " style="height:30mm;">
                <div class="row">
                    <table class="table text-center">
                        <tr>
                            <td>
                                {{ $t('ProductDiscountSuppliersPrintReport.ReceivedBy') }}:
                            </td>
                            <td>
                                {{ $t('ProductDiscountSuppliersPrintReport.ApprovedBy') }}:
                            </td>
                            <td>
                                {{ $t('ProductDiscountSuppliersPrintReport.PreparedBy') }}:
                            </td>
                        </tr>
...
<script>
    ...
    export default {
        props:['printDetails', 'isShown', 'formName', 'isPrint', 'dates'],
        data: function () {
            return {
                list: {
                    inventoryList:
