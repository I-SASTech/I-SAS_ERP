<template>
    <div  v-if="isValid('CanViewCustomersWiseProductSale')" v-bind:style="$i18n.locale == 'ar' ? languageChange('en') : languageChange('ar')">
        <div >
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group ">
                                    <h5 class="card-title DayHeading ">{{ $t('CustomersWiseProductSaleReport.CustomersWiseProductSaleReport') }}</h5>
                                </div>
                            </div>
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group  " v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? 'text-right' : 'text-left'">
                                    ...
                                    <a href="javascript:void(0)"  v-if="isValid('CanPrintCustomersWiseProductSale')" class="btn btn-outline-primary " v-on:click="PrintDetails">{{ $t('CustomersWiseProductSaleReport.Print') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row" v-bind:key="render" v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? '' : 'pr-3'">
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ml-3 ">
                                <label>{{ $t('CustomersWiseProductSaleReport.FromDate') }}</label>
                                <datepicker v-model="fromDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ">
                                <label>{{ $t('CustomersWiseProductSaleReport.ToDate') }}</label>
                                <datepicker v-model="toDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ">
                                <label>{{ $t('CustomersWiseProductSaleReport.ProductName') }}</label>
                                ...
                                <table class="table table-striped table-hover table_list_bg">
                                    <thead class="">
                                        <tr>
                                            <th>#</th>
                                            <th>
                                                {{ $t('CustomersWiseProductSaleReport.TransactionType') }}
                                            </th>
                                            <th>
                                                {{ $t('CustomersWiseProductSaleReport.Customer') }}
                                            </th>
                                            <th>
                                                {{ $t('CustomersWiseProductSaleReport.Quantity') }}
                                            </th>
                                            <th>
                                                {{ $t('CustomersWiseProductSaleReport.SalePrice') }}
                                            </th>
                                            <th>
                                                {{ $t('CustomersWiseProductSaleReport.LineTotal') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(product,index) in productList" v-bind:key="product.id">
                                            ...
                                            <td>
                                                {{product.quantityOut * product.salePrice}}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                productList: [],
                isShown: false,
                formName: 'Customers Wise Product Sale Report',
                printRender: 0,
                combineDate: '',
                language: 'Nothing',
                isMultiUnit: '',
        ...
        methods: {
            languageChange: function (lan) {
                if (this.language == lan) {
                    ...
                    this.$router.go('/CustomersWiseProductSaleReport');
            ...
            productValue: function (x) {
                var productid = x;
                this.GetInventoryList(this.fromDate, this.toDate, 1, productid);
            },
            ...
            GetInventoryList: function (fromdate, todate, currentPage, productId) {  
                ...
                this.$https.get('/Company/GetInventoryList?fromDate=' + fromdate + '&toDate='+ todate + '&pageNumber=' + currentPage + '&isCustomersWP=true' + '&productId=' + productId, { headers: { "Authorization": `Bearer ${token}` } })
            ...
            getTransactionType(transactionType){
                if (transactionType == 'StockOut') return this.$t('CustomersWiseProductSaleReport.StockOut') 
                if (transactionType == 'StockIn') return this.$t('CustomersWiseProductSaleReport.StockIn')
                if (transactionType == 'SaleInvoice') return this.$t('CustomersWiseProductSaleReport.SaleInvoice')
                if (transactionType == 'PurchaseReturn') return this.$t('CustomersWiseProductSaleReport.PurchaseReturn')
                if (transactionType == 'PurchasePost') return this.$t('CustomersWiseProductSaleReport.Purchase')
                if (transactionType == 'CashReceipt') return this.$t('CustomersWiseProductSaleReport.Cash')
            },
