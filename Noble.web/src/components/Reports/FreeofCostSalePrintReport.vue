            ...
            <div class="border border-dark col-md-12 my-1 " style="height:200mm;">
                ...
                <div class="row ">
                    ...
                    <h6 class="m-auto" v-if="isShown && list.inventoryList.length > 0">
                        {{ $t('FreeofCostSalePrintReport.CustomerName') }} : {{list.inventoryList[0].customerName}}
                    </h6>
                    <table class="table col-md-11 m-auto">
                        <tr class="heading" style="font-size:14px;">
                            <th class="text-left">#</th>
                            <th class="text-left">{{ $t('FreeofCostSalePrintReport.Date') }}</th>
                            <th class="text-left">{{ $t('FreeofCostSalePrintReport.ProductName') }}</th>
                            <th class="text-right">{{ $t('FreeofCostSalePrintReport.QuantityOut') }}</th>
                            <th class="text-right">{{ $t('FreeofCostSalePrintReport.SalePrice') }}</th>
                            <th class="text-right">{{ $t('FreeofCostSalePrintReport.LineTotal') }}</th>
                        </tr>
                        <tr style="font-size:13px;" v-for="(item, index) in list.inventoryList" v-bind:key="item.id">
                            ...
                            <td class="text-right">
                                {{item.quantityOut * item.salePrice}}
                            </td>
                        </tr>
                    </table>
                </div>
            ...
            <div class="col-md-12 " style="height:40mm;">
                <div class="row">
                    ...
                    <div class="col-md-4 p-0" style="height:10mm;">
                        <div class="col-md-12" style="height:10mm;">
                            <div class="row text-right">
                                <div class="col-md-12 border-dark border-top border-right border-left border-bottom" style="height:10mm;"> Total Price: {{ calulateTotalPrice }}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-12  " style="height:30mm;">
                <div class="row">
                    <table class="table text-center">
                        <tr>
                            <td>
                                {{ $t('FreeofCostSalePrintReport.ReceivedBy') }}
                            </td>
                            <td>
                                {{ $t('FreeofCostSalePrintReport.ApprovedBy') }}
                            </td>
                            <td>
                                {{ $t('FreeofCostSalePrintReport.PreparedBy') }}
                            </td>
                        </tr>
...
<script>
    ...
    export default {
        props:['printDetails', 'isShown', 'isPrint', 'dates', 'formName'],
        data: function () {
            return {
                list: {
                    inventoryList:
