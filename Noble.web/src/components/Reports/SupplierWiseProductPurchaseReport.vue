<template>
    <div v-if="isValid('CanViewSupplierWiseProductsPurchase')" v-bind:style="$i18n.locale == 'ar' ? languageChange('en') : languageChange('ar')">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group ">
                                    <h5 class="card-title DayHeading ">{{ $t('SupplierWiseProductPurchaseReport.SupplierWiseProductsSaleReport') }}</h5>
                                </div>
                            </div>
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group" v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? 'text-right' : 'text-left'">
                                    ...
                                    <a href="javascript:void(0)" v-if="isValid('CanPrintSupplierWiseProductsPurchase')" class="btn btn-outline-primary " v-on:click="PrintDetails">{{ $t('SupplierWiseProductPurchaseReport.Print') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row" v-bind:key="render" v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? '' : 'pr-3'">
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ml-3">
                                <label>{{ $t('SupplierWiseProductPurchaseReport.FromDate') }}</label>
                                <datepicker v-model="fromDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group">
                                <label>{{ $t('SupplierWiseProductPurchaseReport.ToDate') }}</label>
                                <datepicker v-model="toDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group">
                                <label>{{ $t('SupplierWiseProductPurchaseReport.SupplierName') }}</label>
                                <supplierdropdown v-model="supplierId" :modelValue="supplierId" @update:modelValue="supplierValue(supplierId)" :key="render" />
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                                ...
                                <table class="table table-striped table-hover table_list_bg">
                                    <thead class="">
                                        <tr>
                                            <th>#</th>
                                            <th>
                                                {{ $t('SupplierWiseProductPurchaseReport.TransactionType') }}
                                            </th>
                                            <th>
                                                {{ $t('SupplierWiseProductPurchaseReport.Product') }}
                                            </th>
                                            <th>
                                                {{ $t('SupplierWiseProductPurchaseReport.Quantity') }}
                                            </th>
                                            <th>
                                                {{ $t('SupplierWiseProductPurchaseReport.PurchasePrice') }}
                                            </th>
                                            <th>
                                                {{ $t('SupplierWiseProductPurchaseReport.LineTotal') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(product,index) in productList" v-bind:key="product.id">
                                            ...
                                            <td>
                                                {{product.transactionType=='StockOut'?'Stock Out':product.transactionType=='StockIn'?'Stock In':product.transactionType=='SaleInvoice'?'Sale Invoice':product.transactionType=='PurchaseReturn'?'Purchase Return':product.transactionType=='PurchasePost'?'Purchase':product.transactionType=='CashReceipt'?'Cash':product.transactionType=='CashReceipt'?'Cash':''}}
                                            </td>
                                            <td>
                                                {{product.productName}}
                                            </td>
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                productList: [],
                isShown: false,
                formName: 'Supplier Wise Products Purchase Report',
                printRender: 0,
                combineDate: '',
                language: 'Nothing',
                isMultiUnit: '',
        ...
        methods: {
            languageChange: function (lan) {
                if (this.language == lan) {
                    ...
                    this.$router.go('/SupplierWiseProductPurchaseReport');

                }


            },
            supplierValue: function (x) {
                var supplierid = x;
                this.GetInventoryList(this.fromDate, this.toDate, this.currentPage, supplierid);
            },
            getDate: function (date) {
                return moment(date).format('l');
            },
            getPage: function () {
                this.GetInventoryList(this.fromDate, this.toDate, this.currentPage, this.supplierId);
            },
            GetInventoryList: function (fromdate, todate, currentPage, supplierId) {  
                var root = this;
                var token = '';
                if (token == '') {
                    token = localStorage.getItem('token');
                }
                this.$https.get('/Company/GetInventoryList?fromDate=' + fromdate + '&toDate='+ todate + '&pageNumber=' + currentPage + '&isSWProducts=true' + '&supplierId=' + supplierId, { headers: { "Authorization": `Bearer ${token}` } })
