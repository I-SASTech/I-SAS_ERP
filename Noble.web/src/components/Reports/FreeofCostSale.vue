<template>
    <div  v-if="isValid('CanViewFreeOfCostSale')" v-bind:style="$i18n.locale == 'ar' ? languageChange('en') : languageChange('ar')">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group">
                                    <h5 class="card-title DayHeading">{{ $t('FreeofCostSale.FreeofCostSale') }}</h5>
                                </div>
                            </div>
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group " v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? 'text-right' : 'text-left'">
                                    ...
                                    <a href="javascript:void(0)" v-if="isValid('CanPrintFreeOfCostSale')" class="btn btn-outline-primary " v-on:click="PrintDetails">{{ $t('FreeofCostSale.Print') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row" v-bind:key="render" v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? '' : 'pr-3'">
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ml-3">
                                <label>{{ $t('FreeofCostSale.FromDate') }}</label>
                                <datepicker v-model="fromDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group">
                                <label>{{ $t('FreeofCostSale.ToDate') }}</label>
                                <datepicker v-model="toDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group">
                                <label>{{ $t('FreeofCostSale.CustomerName') }}</label>
                                ...
                                <table class="table table-striped table-hover table_list_bg">
                                    <thead class="">
                                        <tr>
                                            <th>#</th>
                                            <th>
                                                {{ $t('FreeofCostSale.Product') }}
                                            </th>
                                            <th>
                                                {{ $t('FreeofCostSale.QuantityOut') }}
                                            </th>
                                            <th>
                                                {{ $t('FreeofCostSale.SalePrice') }}
                                            </th>
                                            <th>
                                                {{ $t('FreeofCostSale.LineTotal') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
        ...
        <freeofCostSalePrintReport :printDetails="productList" :dates="combineDate" :formName="formName" :isPrint="isShown" :isShown="advanceFilters" v-if="productList.length != 0" v-bind:key="printRender"></freeofCostSalePrintReport>
    </div>
    ...
    export default {
        ...
        methods: {
            ...
            customerValue: function (x) {
                var customerid = x;
                this.GetInventoryList(this.fromDate, this.toDate, 1, customerid);
            },
            ...
            GetInventoryList: function (fromdate, todate, currentPage, customerId) {  
                ...
                this.$https.get('/Company/GetInventoryList?fromDate=' + fromdate + '&toDate='+ todate + '&pageNumber=' + currentPage + '&focSale=true' + '&customerId=' + customerId, { headers: { "Authorization": `Bearer ${token}` } })
