<template>
    <div v-if="isValid('CanViewProductDiscountSupplier')" v-bind:style="$i18n.locale == 'ar' ? languageChange('en') : languageChange('ar')">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group">
                                    <h5 class="card-title DayHeading">{{ $t('ProductDiscountSuppliers.ProductDiscountSuppliers') }}</h5>
                                </div>
                            </div>
                            <div class="col-md-6 col-lg-6" v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? 'text-right' : 'text-left'">
                                <div class="form-group">
                                    ...
                                    <a href="javascript:void(0)" v-if="isValid('CanPrintProductDiscountSupplier')"  class="btn btn-outline-primary " v-on:click="PrintDetails">{{ $t('ProductDiscountSuppliers.Print') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row" v-bind:key="render" v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? '' : 'pr-3'">
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ml-3">
                                <label>{{ $t('ProductDiscountSuppliers.FromDate') }}</label>
                                <datepicker v-model="fromDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group">
                                <label>{{ $t('ProductDiscountSuppliers.ToDate') }}</label>
                                <datepicker v-model="toDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-2 col-lg-2">
                            <div class="form-group">
                                <label>{{ $t('ProductDiscountSuppliers.ProductName') }}</label>
                                <product-single-dropdown v-model="productId" :modelValue="productId" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="btn-group">
                                <button type="button" class="btn btn-outline-primary " v-on:click="valueChange(true)">{{ $t('ProductDiscountSuppliers.FixedDiscount') }}</button>
                                <button type="button" class="btn btn-outline-primary " v-on:click="valueChange(false)">{{ $t('ProductDiscountSuppliers.%Discount') }}</button>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                                ...
                                <table class="table table-striped table-hover table_list_bg">
                                    <thead class="">
                                        <tr>
                                            <th>#</th>
                                            <th>
                                                {{ $t('ProductDiscountSuppliers.Supplier') }}
                                            </th>
                                            <th>
                                                {{ $t('ProductDiscountSuppliers.QuantityIn') }}
                                            </th>
                                            <th>
                                                {{ $t('ProductDiscountSuppliers.PurchasePrice') }}
                                            </th>
                                            <th v-if="advanceFilters">
                                                {{ $t('ProductDiscountSuppliers.FixedDiscount') }}
                                            </th>
                                            <th v-if="!advanceFilters">
                                                {{ $t('ProductDiscountSuppliers.PercentageDiscount') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
        ...
        <productDiscountSuppliersPrintReport :printDetails="productList" :dates="combineDate" :formName="formName" :isPrint="isShown" :isShown="advanceFilters" v-if="productList.length != 0" v-bind:key="printRender"></productDiscountSuppliersPrintReport>
    </div>
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                productList: [],
                isShown: false,
                formName: 'Product Discount Suppliers',
                printRender: 0,
                advanceFilters: true,
                combineDate: '',
                language: 'Nothing',
        ...
        methods: {
            languageChange: function (lan) {
                if (this.language == lan) {
                    ...
                    this.$router.go('/ProductDiscountSuppliers');
            ...
            GetInventoryList: function (fromdate, todate, currentPage, productId, advanceFilters) {  
                ...
                this.$https.get('/Company/GetInventoryList?fromDate=' + fromdate + '&toDate='+ todate + '&pageNumber=' + currentPage + '&isProductSuppliersDiscount=true' + '&isDiscount=' + advanceFilters + '&productId=' + productId, { headers: { "Authorization": `Bearer ${token}` } })
