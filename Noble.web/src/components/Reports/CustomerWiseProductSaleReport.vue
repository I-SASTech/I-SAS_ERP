<template>
    <div  v-if="isValid('CanViewCustomerWiseProductsSale')" v-bind:style="$i18n.locale == 'ar' ? languageChange('en') : languageChange('ar')">
        <div >
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group ">
                                    <h5 class="card-title DayHeading ">{{ $t('CustomerWiseProductSaleReport.CustomersWiseProductSaleReport') }}</h5>
                                </div>
                            </div>
                            <div class="col-md-6 col-lg-6">
                                <div class="form-group " v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? 'text-right' : 'text-left'">
                                    ...
                                    <a href="javascript:void(0)" v-if="isValid('CanPrintCustomerWiseProductsSale')" class="btn btn-outline-primary " v-on:click="PrintDetails">{{ $t('CustomerWiseProductSaleReport.Print') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row" v-bind:key="render" v-bind:class="($i18n.locale == 'en' ||isLeftToRight()) ? '' : 'pr-3' ">
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ml-3 ">
                                <label>{{ $t('CustomerWiseProductSaleReport.FromDate') }}</label>
                                <datepicker v-model="fromDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group ">
                                <label>{{ $t('CustomerWiseProductSaleReport.ToDate') }}</label>
                                <datepicker v-model="toDate" :key="render" />
                            </div>
                        </div>
                        <div class="col-md-3 col-lg-3">
                            <div class="form-group">
                                <label>{{ $t('CustomerWiseProductSaleReport.CustomerName') }}</label>
                                <customerdropdown v-model="customerId" :modelValue="customerId" @update:modelValue="customerValue(customerId)" :key="render" />
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                                ...
                                <table class="table table-striped table-hover table_list_bg">
                                    <thead class="">
                                        <tr>
                                            <th>#</th>
                                            <th>
                                                {{ $t('CustomerWiseProductSaleReport.TransactionType') }}
                                            </th>
                                            <th>
                                                {{ $t('CustomerWiseProductSaleReport.Product') }}
                                            </th>
                                            <th>
                                                {{ $t('CustomerWiseProductSaleReport.Quantity') }}
                                            </th>
                                            <th>
                                                {{ $t('CustomerWiseProductSaleReport.SalePrice') }}
                                            </th>
                                            <th>
                                                {{ $t('CustomerWiseProductSaleReport.LineTotal') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                productList: [],
                isShown: false,
                formName: 'Customer Wise Products Sale Report',
                printRender: 0,
                combineDate: '',
                language: 'Nothing',
                isMultiUnit: '',
        ...
        methods: {
            languageChange: function (lan) {
                if (this.language == lan) {
                    ...
                    this.$router.go('/CustomerWiseProductSaleReport');

                }


            },
            customerValue: function (x) {
                var customerid = x;
                this.GetInventoryList(this.fromDate, this.toDate, this.currentPage, customerid);
            },
            getDate: function (date) {
                return moment(date).format('l');
            },
            getPage: function () {
                this.GetInventoryList(this.fromDate, this.toDate, this.currentPage, this.customerId);
            },
            GetInventoryList: function (fromdate, todate, currentPage, customerId) {  
                var root = this;
                var token = '';
                if (token == '') {
                    token = localStorage.getItem('token');
                }
                this.$https.get('/Company/GetInventoryList?fromDate=' + fromdate + '&toDate='+ todate + '&pageNumber=' + currentPage + '&isCWProducts=true' + '&customerId=' + customerId, { headers: { "Authorization": `Bearer ${token}` } })
            ...
            getTransactionType(transactionType){
                if (transactionType == 'StockOut') return this.$t('CustomerWiseProductSaleReport.StockOut') 
                if (transactionType == 'StockIn') return this.$t('CustomerWiseProductSaleReport.StockIn')
                if (transactionType == 'SaleInvoice') return this.$t('CustomerWiseProductSaleReport.SaleInvoice')
                if (transactionType == 'PurchaseReturn') return this.$t('CustomerWiseProductSaleReport.PurchaseReturn')
                if (transactionType == 'PurchasePost') return this.$t('CustomerWiseProductSaleReport.Purchase')
                if (transactionType == 'CashReceipt') return this.$t('CustomerWiseProductSaleReport.Cash')
            },
