<template>
    <div>
        <multiselect v-model="DisplayValue"
                     ...
                     label="name"
                     :disabled="disable"
                     v-bind:placeholder="$t('AddPurchaseOrder.SelectPurchaseOrder')"
    ...
    export default {
        ...
        methods: {
            getData: function () {
                ...
                this.$https.get('/Purchase/PurchaseTemplateList?IsDropDown=' + true + '&branchId=' + branchId, { headers: { Authorization: `Bearer ${token}` }, })
