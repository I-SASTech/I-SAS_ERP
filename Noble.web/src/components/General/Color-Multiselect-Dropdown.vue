    ...
    export default {
        ...
        methods: {
            getData: function () {
                ...
                this.$https.get('/Product/ColorList?isActive=true', { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
                    ...
                    if (root.modelValue != null && root.modelValue != undefined && root.modelValue != '') {
                        root.modelValue.forEach(function (id) {
                            var color = root.options.find(function (x) { return x.id == id; });
                            root.value.push(color);
                        });
