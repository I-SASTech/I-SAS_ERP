<template>
    <div>
        <multiselect v-model="DisplayValue" :options="options" :disabled="disabled" :multiple="false"
            v-bind:placeholder="$t('UnitDropdown.SelectUnit')" track-by="name" :clear-on-select="false"
            ...
            <template v-slot:noResult>
                <span  class="btn btn-primary " v-on:click="AddUnit('Add')" v-if="isValid('CanAddUnit')">{{
                    $t('UnitDropdown.AddProductUnit')
            }}</span><br />
            </template>
        </multiselect>
                ...
                <div class="modal-body">
                    <div class="row ">
                        <div :key="render" class="form-group has-label col-sm-12 "
                            ...
                            <label class="text  font-weight-bolder"> {{ $t('UnitDropdown.Code') }}:<span
                        ...
                        <div v-if="english == 'true'" class="form-group has-label col-sm-12 "
                            ...
                            <label class="text  font-weight-bolder"> {{$englishLanguage($t('UnitDropdown.UnitName')) 
                        ...
                        <div v-if="isOtherLang()" class="form-group has-label col-sm-12 "
                            ...
                            <label class="text  font-weight-bolder"> {{$arabicLanguage($t('UnitDropdown.UnitNameAr')) 
                                    
                            }}: <span class="text-danger"> *</span></label>
                            <input class="form-control " v-bind:class="isLeftToRight() ? 'text-left' : 'arabicLanguage'"
                                v-model="v$.unit.nameArabic.$model" type="text" />
                            <span v-if="v$.unit.nameArabic.$error" class="error">
                                <span v-if="!v$.unit.nameArabic.required">{{ $t('UnitDropdown.NameRequired')
                                }}</span>
                                <span v-if="!v$.unit.nameArabic.maxLength">{{ $t('UnitDropdown.NameLength')
                                }}</span>
                            </span>
                        </div>
                        ...
                        <div class="form-group col-md-4">
                            <div class="checkbox form-check-inline mx-2">
                                ...
                                <label for="inlineCheckbox1"> {{ $t('UnitDropdown.Active') }} </label>
                            </div>
                        </div>
                ...
                <div v-if="!loading">
                    <div class="modal-footer justify-content-right" v-if="type == 'Edit'">
                        <button type="button" class="btn btn-soft-primary btn-sm" v-on:click="SaveUnit"
                                v-bind:disabled="v$.unit.$invalid">
                            {{ $t('UnitDropdown.Update') }}
                        </button>
                        <button type="button" class="btn btn-soft-secondary btn-sm" v-on:click="close()">{{ $t('UnitDropdown.btnClear') }}</button>
                    </div>
                    <div class="modal-footer justify-content-right" v-else>
                        <button type="button" class="btn btn-soft-primary btn-sm" v-on:click="SaveUnit"
                            v-bind:disabled="v$.unit.$invalid"> {{ $t('UnitDropdown.btnSave') }}</button>
                        <button type="button" class="btn btn-soft-secondary btn-sm" v-on:click="close()">{{
                                $t('UnitDropdown.btnClear')
                        }}</button>
                    </div>
                </div>
                <div v-else>
