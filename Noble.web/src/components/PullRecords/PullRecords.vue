<template>
    <div class="row" v-if="isValid('CanPullRecord')">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="page-title-box">
                        <div class="row">
                            <div class="col">
                                <h4 class="page-title">{{ ($t('PullRecords.PullDetails')) }}</h4>
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a
                                            href="javascript:void(0);">{{ ($t('PullRecords.Home')) }}</a></li>
                                    <li class="breadcrumb-item active">{{ ($t('PullRecords.PullDetails')) }}</li>
                                </ol>
                            </div>
                            <div class="col-auto align-self-center">
                                <button class="btn btn btn-sm btn-outline-primary mx-1  " @click="PullData"
                                    :disabled="loading">{{ ($t('PullRecords.Home')) }}</button>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="progress mb-2" v-if="!loading">
                        <div class="progress-bar bg-success" role="progressbar" :style="'width:' + syncedRecords + '%'"
                            aria-valuenow="15" aria-valuemin="0" aria-valuemax="100">{{ Math.floor(syncedRecords) }}%</div>
                        <div class="progress-bar bg-warning" role="progressbar" :style="'width:' + pendingRecords + '%'"
                            aria-valuenow="30" aria-valuemin="0" aria-valuemax="100">{{ Math.ceil(pendingRecords) }}%</div>
                    </div>
                    <div class=" mb-2">
                        <div class="">
                            <ul class="nav nav-tabs" data-tabs="tabs">
                                <li class="nav-item"><a class="nav-link" v-bind:class="{ active: status == 'All' }"
                                        v-on:click="changeStatus('All')" id="v-pills-home-tab" data-toggle="pill"
                                        href="#v-pills-home" role="tab" aria-controls="v-pills-home"
                                        aria-selected="true">{{ ($t('PullRecords.AllRecords')) }}</a></li>
                                <li class="nav-item"><a class="nav-link" v-bind:class="{ active: status == 'Sync' }"
                                        v-on:click="changeStatus('Sync')" id="v-pills-profile-tab" data-toggle="pill"
                                        href="#v-pills-profile" role="tab" aria-controls="v-pills-profile"
                                        aria-selected="false">{{ ($t('PullRecords.Synced')) }}</a></li>
                                <li class="nav-item"><a class="nav-link" v-bind:class="{ active: status == 'NotSync' }"
                                        v-on:click="changeStatus('NotSync')" id="v-pills-profile-tab" data-toggle="pill"
                                        href="#v-pills-profile1" role="tab" aria-controls="v-pills-profile"
                                        aria-selected="false">{{ ($t('PullRecords.Pending')) }}</a></li>
                                    ...
                                    <table class="table mb-0">
                                        <thead class="thead-light table-hover">
                                            <tr>
                                                ...
                                                <th class="text-center">
                                                    {{ ($t('PullRecords.PullDate')) }}
                                                </th>
                                                <th class="text-center">
                                                    {{ ($t('PullRecords.CreatedDate')) }}
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr v-for="details in syncRecordList" v-bind:key="details.id">
                                                ...
                                                <td class="text-center">
                                                    <span
                                                        :class="details.pull == true ? 'badge badge-success' : 'badge badge-warning'">
                                                        {{ details.pull == true ? 'Synced' : 'Pending' }}
                                                    </span>
                                                </td>
                                                <td class="text-center">
                                                    {{ $filterDate(details.pullDate) }}
                                                </td>
