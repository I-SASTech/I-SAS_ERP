...
namespace Focus.Business.WareHouseTransfers.Queries.GetWareHouseTransferRegistrationNo
{
    public class GetWareHouseTransferRegistrationNoQuery : IRequest<string>
    {
        public Guid? BranchId { get; set; }

        public class Handler : IRequestHandler<GetWareHouseTransferRegistrationNoQuery, string>
        {
            private readonly IApplicationDbContext _context;
            private readonly ILogger _logger;

            public Handler(IApplicationDbContext context, ILogger<GetWareHouseTransferRegistrationNoQuery> logger)
            {
                _context = context;
                _logger = logger;
            }

            public async Task<string> Handle(GetWareHouseTransferRegistrationNoQuery request, CancellationToken cancellationToken)
            {
                try
                {
                    var branchCode = "";
