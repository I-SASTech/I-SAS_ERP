...
{
    public class StockTransferAutoNumber : IRequest<string>
    {
        public Guid? BranchId { get; set; }

        public class Handler : IRequestHandler<StockTransferAutoNumber, string>
        {
            private readonly IApplicationDbContext _context;
            private readonly ILogger _logger;

            public Handler(IApplicationDbContext context, ILogger<StockTransferAutoNumber> logger)
            {
                _context = context;
                _logger = logger;
            }

            public async Task<string> Handle(StockTransferAutoNumber request, CancellationToken cancellationToken)
            {
                try
                {
                    var branchCode = "";
                    ...
                    {
                        wareHouseTransfer = await _context.StockTransfers
